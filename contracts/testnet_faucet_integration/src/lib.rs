...
impl TestnetFaucetIntegration {
    ...
    pub fn initialize(env: Env, admin: Address, token_address: Address) {
        ...
        env.storage().instance().set(&DataKey::DailyLimit, &DEFAULT_DAILY_LIMIT);
        env.storage().instance().set(&DataKey::RequestLimit, &DEFAULT_REQUEST_LIMIT);
    ...
    pub fn request_tokens(
        ...
        // Validate amount
        let request_limit: i128 = env.storage().instance().get(&DataKey::RequestLimit).unwrap();
        ...
        if let Some(last_request_time) = env.storage().persistent().get::<DataKey, u64>(&last_request_key) {
        ...
        let request_id: u64 = env.storage().instance().get(&DataKey::RequestCounter).unwrap();
        ...
        env.storage().persistent().set(&DataKey::FaucetRequest(request_id), &request);
        env.storage().persistent().set(&last_request_key, &env.ledger().timestamp());
        env.storage().instance().set(&DataKey::RequestCounter, &(request_id + 1));
        ...
        // Emit event
        env.events().publish((String::from_str(&env, "faucet_req"),), (request_id, user, amount));
    ...
    pub fn create_project(
        ...
        let project_id: u64 = env.storage().instance().get(&DataKey::ProjectCounter).unwrap();
        ...
        env.storage().persistent().set(&DataKey::ProjectIdea(project_id), &project);
        env.storage().instance().set(&DataKey::ProjectCounter, &(project_id + 1));

        env.events().publish((String::from_str(&env, "project_created"),), (project_id, creator));
    ...
    pub fn get_project(env: Env, project_id: u64) -> Option<ProjectIdea> {
        env.storage().persistent().get(&DataKey::ProjectIdea(project_id))
    }

    /// List all projects
    pub fn list_projects(env: Env) -> Vec<u64> {
        let count: u64 = env.storage().instance().get(&DataKey::ProjectCounter).unwrap_or(0);
    ...
    pub fn get_request(env: Env, request_id: u64) -> Option<FaucetRequest> {
        env.storage().persistent().get(&DataKey::FaucetRequest(request_id))
    }
    ...
    pub fn set_request_limit(env: Env, new_limit: i128) {
        ...
        env.storage().instance().set(&DataKey::RequestLimit, &new_limit);
    }
    ...
    pub fn fund_faucet(env: Env, funder: Address, amount: i128) {
        ...
        env.events().publish((String::from_str(&env, "funded"),), (funder, amount));
    }
    ...
    pub fn get_request_limit(env: Env) -> i128 {
        env.storage().instance().get(&DataKey::RequestLimit).unwrap()
    }
}
