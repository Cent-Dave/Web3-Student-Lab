...
fn test_create_team() {
    ...
    let req_skills = vec![&env, Symbol::new(&env, "react"), Symbol::new(&env, "stellar")];
    let req_roles = vec![&env, Symbol::new(&env, "frontend"), Symbol::new(&env, "designer")];
...
fn test_join_request_flow() {
    ...
    let team_id = client.create_team(&creator, &Symbol::new(&env, "T1"), &vec![&env], &vec![&env], &2);
    
    // Register applicant
    let applicant = Address::generate(&env);
    client.register_developer(&applicant, &vec![&env, Symbol::new(&env, "rust")], &Symbol::new(&env, "backend"));
...
fn test_join_request_team_full() {
    ...
    let creator = Address::generate(&env);
    client.register_developer(&creator, &vec![&env], &Symbol::new(&env, "creator"));
    let team_id = client.create_team(&creator, &Symbol::new(&env, "T1"), &vec![&env], &vec![&env], &1);
...
fn test_invitation_flow() {
    ...
    let creator = Address::generate(&env);
    client.register_developer(&creator, &vec![&env], &Symbol::new(&env, "creator"));
    let team_id = client.create_team(&creator, &Symbol::new(&env, "T1"), &vec![&env], &vec![&env], &3);
...
fn test_leave_and_remove_member() {
    ...
    let creator = Address::generate(&env);
    client.register_developer(&creator, &vec![&env], &Symbol::new(&env, "creator"));
    let team_id = client.create_team(&creator, &Symbol::new(&env, "T1"), &vec![&env], &vec![&env], &4);
...
fn test_matching_engine() {
    ...
    let team_id1 = client.create_team(
        ...
        &vec![&env, Symbol::new(&env, "rust"), Symbol::new(&env, "stellar")],
