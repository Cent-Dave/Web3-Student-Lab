...
const KEY_FILES: Symbol = soroban_sdk::symbol_short!("files");
const KEY_SHARDS: Symbol = soroban_sdk::symbol_short!("shards");
const KEY_PROVIDERS: Symbol = soroban_sdk::symbol_short!("providers");
...
impl DecentralizedStorage {
    pub fn initialize(env: Env) {
        if env.storage().instance().has(&KEY_FILES) {
            panic!("Already initialized");
        }
        env.storage()
            .instance()
            .set(&KEY_FILES, &Map::<BytesN<32>, StoredFile>::new(&env));
        env.storage()
            .instance()
            .set(&KEY_SHARDS, &Map::<BytesN<32>, Vec<FileShard>>::new(&env));
        env.storage()
            .instance()
            .set(&KEY_PROVIDERS, &Map::<Address, StorageProvider>::new(&env));
    }

    /// Register as a storage provider
    pub fn register_provider(env: Env, provider: Address, available_space: u64) {
        provider.require_auth();
        let mut providers: Map<Address, StorageProvider> =
            env.storage().instance().get(&KEY_PROVIDERS).unwrap();
        providers.set(
            provider.clone(),
            StorageProvider {
                address: provider,
    ...
    pub fn upload_file(
        ...
        encryption_key: BytesN<32>,
        redundancy: u32,
    ) -> BytesN<32> {
        owner.require_auth();
        let total_size = encrypted_data.len() as u32;
        let shard_count = (total_size + SHARD_SIZE - 1) / SHARD_SIZE;
        let file_id: BytesN<32> = env.crypto().sha256(&encrypted_data).into();

        let mut files: Map<BytesN<32>, StoredFile> =
            env.storage().instance().get(&KEY_FILES).unwrap();
        files.set(
            file_id.clone(),
            StoredFile {
                id: file_id.clone(),
        ...
        let mut providers: Map<Address, StorageProvider> =
            env.storage().instance().get(&KEY_PROVIDERS).unwrap();
        let mut shards: Map<BytesN<32>, Vec<FileShard>> =
            env.storage().instance().get(&KEY_SHARDS).unwrap();
    ...
    pub fn get_shards(env: Env, file_id: BytesN<32>) -> Vec<FileShard> {
        let shards: Map<BytesN<32>, Vec<FileShard>> =
            env.storage().instance().get(&KEY_SHARDS).unwrap();
    ...
    pub fn verify_file(env: Env, file_id: BytesN<32>) -> bool {
        ...
        if file_shards.is_none() {
            return false;
        }
        for shard in file_shards.unwrap().iter() {
            let provider: StorageProvider = env
                .storage()
                .instance()
                .get(&KEY_PROVIDERS)
                .unwrap_or(Map::new(&env))
                .get(shard.provider.clone())
                .unwrap_or(StorageProvider {
                    address: shard.provider.clone(),
                    total_stored: 0,
                    available_space: 0,
                    shard_count: 0,
                    reputation: 0,
            ...
            if provider.last_heartbeat + 86400 < env.ledger().timestamp() {
                return false;
            }
        }
        true
