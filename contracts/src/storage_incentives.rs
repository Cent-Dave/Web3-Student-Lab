...
const KEY_PAYMENTS: Symbol = soroban_sdk::symbol_short!("payments");
const KEY_PROOFS: Symbol = soroban_sdk::symbol_short!("proofs");
const KEY_EARNINGS: Symbol = soroban_sdk::symbol_short!("earnings");
...
impl StorageIncentives {
    pub fn initialize(env: Env) {
        if env.storage().instance().has(&KEY_PAYMENTS) {
            panic!("Already initialized");
        }
        env.storage()
            .instance()
            .set(&KEY_PAYMENTS, &Vec::<(Address, i128, u64)>::new(&env));
        env.storage()
            .instance()
            .set(&KEY_PROOFS, &Vec::<StorageProof>::new(&env));
        env.storage()
            .instance()
            .set(&KEY_EARNINGS, &Map::<Address, ProviderEarnings>::new(&env));
    }
    ...
    pub fn submit_proof(
        ...
        let mut proofs: Vec<StorageProof> = env
            .storage()
            .instance()
            .get(&KEY_PROOFS)
        ...
        // Reward provider
        let mut earnings: Map<Address, ProviderEarnings> = env
            .storage()
            .instance()
            .get(&KEY_EARNINGS)
        ...
        // Record payment
        let mut payments: Vec<(Address, i128, u64)> = env
            .storage()
            .instance()
            .get(&KEY_PAYMENTS)
            .unwrap_or(Vec::new(&env));
    ...
    pub fn calculate_storage_payment(
        env: Env,
        provider: Address,
        bytes_stored: u64,
        days: u64,
    ...
    pub fn apply_penalty(env: Env, provider: Address) {
        ...
        env.events().publish(
            ...
            PaymentDistributedEvent {
                provider,
                amount: -penalty,
                reason: Symbol::new(&env, "penalty"),
                timestamp: env.ledger().timestamp(),
            },
        );
    }
    ...
    pub fn get_provider_earnings(env: Env, provider: Address) -> ProviderEarnings {
        let earnings: Map<Address, ProviderEarnings> = env
            .storage()
            .instance()
            .get(&KEY_EARNINGS)
            .unwrap_or(Map::new(&env));
    ...
    pub fn get_file_proofs(env: Env, file_id: soroban_sdk::BytesN<32>) -> Vec<StorageProof> {
        let proofs: Vec<StorageProof> = env
            .storage()
            .instance()
            .get(&KEY_PROOFS)
            .unwrap_or(Vec::new(&env));
    ...
    pub fn get_payment_history(env: Env, provider: Address) -> Vec<(Address, i128, u64)> {
        let payments: Vec<(Address, i128, u64)> = env
            .storage()
            .instance()
            .get(&KEY_PAYMENTS)
            .unwrap_or(Vec::new(&env));
        ...
        for (addr, amount, timestamp) in payments.iter() {
            if addr == provider {
                result.push_back((addr, amount, timestamp));
            }
        }
        result
    }
