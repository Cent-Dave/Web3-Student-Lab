...
const KEY_TOTAL_SUPPLY: Symbol = soroban_sdk::symbol_short!("totalsupp");
const KEY_BURN_PERCENTAGE: Symbol = soroban_sdk::symbol_short!("burn_pct");
const KEY_BURN_HISTORY: Symbol = soroban_sdk::symbol_short!("burn_hist");
const KEY_BALANCES: Symbol = soroban_sdk::symbol_short!("balances");
...
impl DeflationaryToken {
    pub fn initialize(env: Env, initial_supply: i128, burn_percentage: u32) {
        if env.storage().instance().has(&KEY_TOTAL_SUPPLY) {
            panic!("Already initialized");
        }
        if burn_percentage > 1000 {
            panic!("Burn % max 10% (1000 bps)");
        }
        env.storage()
            .instance()
        ...
        env.storage()
            .instance()
            .set(&KEY_BURN_PERCENTAGE, &burn_percentage);
        env.storage()
            .instance()
            .set(&KEY_BURN_HISTORY, &Map::<u64, BurnRecord>::new(&env));
        env.storage()
            .instance()
            .set(&KEY_BALANCES, &Map::<Address, i128>::new(&env));
    }
    ...
    pub fn transfer(env: Env, from: Address, to: Address, amount: i128) {
        ...
        let burn_pct: u32 = env
            .storage()
            .instance()
            .get(&KEY_BURN_PERCENTAGE)
            .unwrap_or(0);
        ...
        if burn_amount > 0 {
            ...
            let mut history: Map<u64, BurnRecord> =
                env.storage().instance().get(&KEY_BURN_HISTORY).unwrap();
    ...
    pub fn mint(env: Env, to: Address, amount: i128) {
        ...
        env.storage()
            .instance()
            .set(&KEY_TOTAL_SUPPLY, &(total_supply + amount));
    }
